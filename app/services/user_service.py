...
class UserService:
    ...
    async def create_user(self, user_data: UserCreate) -> BaseResponse:
        ...
        # 이름 중복 체크
        name_query = User.__table__.select().where(User.name.collate('latin1_swedish_ci') == user_data.name)
        ...
        # 이메일 중복 체크
        email_query = User.__table__.select().where(User.email.collate('latin1_swedish_ci') == user_data.email)
    ...
    async def authenticate_user(self, login_data: UserLogin) -> TokenResponse:
        # 사용자 조회
        query = User.__table__.select().where(User.email.collate('latin1_swedish_ci') == login_data.email)
